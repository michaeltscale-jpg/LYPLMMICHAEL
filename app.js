        ...
        if (g.key === "gate1") {
            ...
            upperActionHtml = `
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; width: 100%;">
                    <button class="btn-secondary" style="font-size: 0.68rem; padding: 4px; display: flex; align-items: center; justify-content: center; gap: 3px;" onclick="openBomDesignerNew()"><i data-lucide="layers" style="width: 11px; height: 11px;"></i> 配方 BOM</button>
...
window.saveNewEquipment = async function() {
    ...
    const now = new Date();
    const fmt = d => d.toISOString().substring(0, 10);
    const addD = (d, days) => { const n = new Date(d); n.setDate(n.getDate() + days); return n; };

    const g1S = fmt(now), g1E = fmt(addD(now, 7));
    const g2S = fmt(addD(g1E, 1)), g2E = fmt(addD(g2S, 10));
    const g3S = fmt(addD(g2E, 1)), g3E = fmt(addD(g3S, 15));
    const g4S = fmt(addD(g3E, 1)), g4E = fmt(addD(g4S, 30));
    const g5S = fmt(addD(g4E, 1)), g5E = fmt(addD(g5S, 15));
...
function renderDmsDeliverablesTable(product) {
    ...
    // SOP与工段规程仅在符合条件的中试(G3)、试产(G4)、量产(G5)阶段输出 (G1立项与G2配方阶段不输出SOP)
    const showSegmentSopSip = (activeFilter === 'G3' || activeFilter === 'G4' || activeFilter === 'G5');
    
    if (showSegmentSopSip) {
        let activeSteps = [];
        ...
        let phaseLabel = "G3-G5 工段规程";
        if (activeFilter === 'G3') phaseLabel = "G3 中试规程";
        else if (activeFilter === 'G4') phaseLabel = "G4 试产规程";
        else if (activeFilter === 'G5') phaseLabel = "G5 量产规程";
        ...
        activeSteps.forEach((r, idx) => {
            ...
            sopTr.innerHTML = `
                <td style="font-size:0.72rem; color:#2563eb; font-weight:700;">${phaseLabel}</td>
            ...
            sipTr.innerHTML = `
                <td style="font-size:0.72rem; color:#059669; font-weight:700;">${phaseLabel}</td>
...
window.switchPDCAStage = function(stage) {
    ...
    const targetIndex = stages.indexOf(stage || maxStage);
    ...
    const currentStage = stage || maxStage;

    const titles = {
        'Plan': '阶段详情: Plan. 问题识别与解决方案',
        'Do': '阶段详情: Do. 实施与排查',
        'Check': '阶段详情: Check. 检查验证效果',
        'Act': '阶段详情: Act. 标准化与闭环调整'
    };
    const reqs = {
        'Plan': '<div style="display:flex; gap:6px;"><i data-lucide="check-circle-2" style="width:12px; height:12px; color:#10b981; margin-top:3px;"></i> <span>需详细阐述不良现象及影响范围。</span></div>',
        'Do': '<div style="display:flex; gap:6px;"><i data-lucide="check-circle-2" style="width:12px; height:12px; color:#10b981; margin-top:3px;"></i> <span>排查人机料法环根因并提供中试计划。</span></div>',
        'Check': '<div style="display:flex; gap:6px;"><i data-lucide="check-circle-2" style="width:12px; height:12px; color:#10b981; margin-top:3px;"></i> <span>提供 OEE 或品质改善数据证明。</span></div>',
    ...
    document.getElementById('stage-detail-title').innerText = titles[currentStage] || '';
    document.getElementById('stage-control-reqs').innerHTML = reqs[currentStage] || '';
    document.getElementById('pdca-edit-stage').value = maxStage;
    if (window.lucide) {
        lucide.createIcons();
    }
};
...
window.exportCpkAnalysisToCsv = function() {
    ...
    const data = [
        ["Batch-06 (中试)", "HF-PTS-12 (12μm)", "50", "0.82", "0.015", "0.65 ~ 1.00", "1.65", "1.58", "A级"],
        ["Batch-05", "HF-PTS-12 (12μm)", "50", "0.81", "0.018", "0.65 ~ 1.00", "1.50", "1.45", "A级"],
        ["Batch-04", "HF-PTS-12 (12μm)", "50", "0.79", "0.021", "0.65 ~ 1.00", "1.38", "1.33", "A级"],
        ["Batch-03", "HF-PTS-12 (12μm)", "50", "0.76", "0.026", "0.65 ~ 1.00", "1.32", "1.28", "B级"],
        ["Batch-02", "HF-PTS-12 (12μm)", "50", "0.74", "0.031", "0.65 ~ 1.00", "1.20", "1.15", "C级"],
        ["Batch-01", "HF-PTS-12 (12μm)", "50", "0.68", "0.045", "0.65 ~ 1.00", "0.98", "0.92", "不合格"]
    ];
...
window.openManualDetailModal = function(tabId) {
    currentManualTargetTab = tabId;
    const modal = document.getElementById("modal-manual-detail");
    const titleEl = document.getElementById("manual-detail-title");
    const bodyEl = document.getElementById("manual-detail-body");
    ...
    const info = detailsMap[tabId] || detailsMap['dashboard-panel'];
...
window.jumpFromManual = function() {
    closeManualDetailModal();
    if (window.switchTab && currentManualTargetTab) {
